...
if [ "${meta}" == *"openclash"* ];then
  ...
  CLASH_META_URL="https://github.com/DustinWin/clash_singbox-tools/releases/download/mihomo/mihomo-meta-linux-armv7.tar.gz"
fi
...
if [ "${meta}" == *"openclash"* ];then
  ...
  curl -sfL -o ./meta.tar.gz $CLASH_META_URL
  tar -zxf ./meta.tar.gz
  mv -f ./CrashCore files/etc/openclash/core/clash_meta
fi
